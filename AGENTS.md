...
- **Transformation**: dbt Core (`dbt-postgres`) under `dbt/mercury_analytics/` with 6 analytical marts (`mart_customer_metrics`, `dim_customers`, `fact_orders`, `mart_product_metrics`, `mart_seller_metrics`, `mart_marketing_funnel`).
- **Machine learning**: scikit-learn (RFM segmentation and Random Forest churn prediction models under `ml/`).
- **API**: FastAPI application under `backend/` exposing 40 REST endpoints across health, observability, analytics, customer intelligence, predictions, retention, products, sellers, and marketing.
- **Testing**: 202 automated tests in `tests/` (164 offline mock/unit tests, 38 live DB integration tests skipped without `DATABASE_URL`).
