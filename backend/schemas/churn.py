...
class ChurnPrediction(BaseModel):
    ...
    is_churned: int = Field(
        ..., description="Observed ground truth inactivity label (1=churned, 0=active)"
    )
    churn_probability: float = Field(
        ..., ge=0.0, le=1.0, description="Predicted probability of churn P(Churn)"
    )
    risk_tier: str = Field(
    ...
    revenue_at_risk: float = Field(
        ..., ge=0.0, description="Expected monetary revenue at risk P(Churn) * monetary_value"
    )
    retention_priority: str = Field(..., description="Decision matrix retention action group")
    predicted_at: Optional[Union[datetime, str]] = Field(
        None, description="Timestamp of inference generation"
    )
...
class RiskTierSummary(BaseModel):
    ...
    total_revenue_at_risk: float = Field(
        ..., ge=0.0, description="Sum of expected revenue at risk in BRL"
    )
    avg_churn_probability: float = Field(
        ..., ge=0.0, le=1.0, description="Mean predicted churn probability"
    )
...
class RetentionPrioritySummary(BaseModel):
    ...
    customer_count: int = Field(
        ..., ge=0, description="Number of customers assigned to this priority"
    )
    percentage: float = Field(..., ge=0.0, le=100.0, description="Percentage of customer portfolio")
    total_revenue_at_risk: float = Field(
        ..., ge=0.0, description="Total revenue at risk tied to this action group"
    )
